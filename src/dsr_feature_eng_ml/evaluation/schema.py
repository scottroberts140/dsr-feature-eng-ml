...
def _normalize_model_params_mapping(model_params: Any) -> dict[str, Any]:
    """Return a serializer-friendly hyperparameter mapping."""
    if model_params is None:
        return {}

    if isinstance(model_params, Mapping):
        raw_params = dict(model_params)
    elif hasattr(model_params, "to_dict"):
        raw_params = model_params.to_dict()
    elif dataclasses.is_dataclass(model_params) and not isinstance(model_params, type):
        raw_params = dataclasses.asdict(cast(Any, model_params))
    else:
        raw_params = dict(vars(model_params))

    params_dict: dict[str, Any] = {}
    for key, value in raw_params.items():
        if value is None:
            continue
        params_dict[str(key)] = value.name if isinstance(value, Enum) else value
...
class DataSplits:
    ...
    def from_data_source(
        ...
        numeric_ohe_suffix_width: int | None = None,
    ) -> DataSplits:
        """
        ...
        numeric_ohe_suffix_width : int, optional
            Width to use when zero-padding integer suffixes generated by
            one-hot encoding for numeric categorical columns. If None,
            width is inferred per column from observed training categories.
            Sentinel category ``-1`` is always renamed to ``Other``.
        """
        if numeric_ohe_suffix_width is not None and numeric_ohe_suffix_width < 1:
            raise ValueError("numeric_ohe_suffix_width must be >= 1 when provided.")
        ...
        if categorical_cols:
            ...
            # Normalize numeric-categorical suffixes so lexical sorting is stable
            # and sentinel -1 becomes a user-friendly "Other" label.
            rename_map: dict[str, str] = {}
            for col in categorical_cols:
                train_series = train_feat[col].dropna()
                if train_series.empty:
                    continue

                numeric_train = pd.to_numeric(train_series, errors="coerce")
                if not numeric_train.notna().all():
                    continue

                numeric_arr = numeric_train.to_numpy(dtype=np.float64)
                if not np.isclose(numeric_arr, np.round(numeric_arr)).all():
                    continue

                max_abs = int(np.max(np.abs(numeric_arr))) if numeric_arr.size else 0
                inferred_width = max(1, len(str(max_abs)))
                width = numeric_ohe_suffix_width or inferred_width

                prefix = f"{col}_"
                for existing_name in train_cat.columns:
                    if not existing_name.startswith(prefix):
                        continue

                    raw_suffix = existing_name[len(prefix) :]
                    numeric_suffix = pd.to_numeric(raw_suffix, errors="coerce")
                    if (
                        pd.isna(numeric_suffix)
                        or not float(numeric_suffix).is_integer()
                    ):
                        continue

                    suffix_int = int(numeric_suffix)
                    normalized_suffix = (
                        "Other" if suffix_int == -1 else f"{suffix_int:0{width}d}"
                    )
                    new_name = f"{col}_{normalized_suffix}"
                    if new_name in train_cat.columns and new_name != existing_name:
                        continue
                    rename_map[existing_name] = new_name

            if rename_map:
                train_cat = train_cat.rename(columns=rename_map)
                val_cat = val_cat.rename(columns=rename_map)
                test_cat = test_cat.rename(columns=rename_map)
...
class ModelConfiguration(Generic[T_Params]):
    ...
    def params_dict(self) -> dict[str, Any]:
        ...
        return _normalize_model_params_mapping(self.model_params)

    @property
    def params_info(self) -> str:
        """Human-readable hyperparameter summary for reports."""
        if hasattr(self.model_params, "info"):
            return self.model_params.info()
    ...
    def info(self) -> str:
        ...
        data.extend([(str(k), str(v)) for k, v in self.params_dict.items()])
...
class ModelAuditorConfig:
    ...
    # Core split state must remain consistent with from_dataset construction.
    _PROTECTED_POST_INIT_FIELDS: ClassVar[frozenset[str]] = frozenset({"data_splits"})

    def __setattr__(self, name: str, value: Any) -> None:
        """Prevent reassignment of protected fields after initialization."""
        if name in self._PROTECTED_POST_INIT_FIELDS and name in self.__dict__:
            raise AttributeError(
                f"'{name}' is read-only after initialization. "
                "Use ModelAuditorConfig.from_dataset(...) to build new splits."
            )
    ...
    @classmethod
    def overridable_fields(cls) -> set[str]:
        """Return the config fields that are safe to override post-construction."""
        dataclass_fields = {
            f.name
            for f in dataclasses.fields(cls)
            if not f.name.startswith("_")
            and f.name not in cls._PROTECTED_POST_INIT_FIELDS
        }
        # n_jobs is managed through a property with propagation/validation.
        dataclass_fields.add("n_jobs")
        return dataclass_fields

    def apply_overrides(self, overrides: Mapping[str, Any] | None) -> list[str]:
        """Apply validated post-construction overrides and return applied keys."""
        if not overrides:
            return []
        ...
        if invalid_keys:
            raise ValueError(
                "ModelAuditorConfig override keys must be non-empty public strings. "
                f"Invalid keys: {sorted(map(str, invalid_keys))}"
            )

        allowed = self.overridable_fields()
        unknown = sorted(k for k in overrides if k not in allowed)
        if unknown:
            raise ValueError(
                "Unsupported ModelAuditorConfig override fields: "
                f"{unknown}. Allowed fields: {sorted(allowed)}"
            )
