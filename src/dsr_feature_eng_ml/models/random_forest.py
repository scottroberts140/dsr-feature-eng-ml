...
import pandas as pd
import numpy as np
from typing import Union, Optional, cast, Mapping, Type, Any, Final, Literal
...
class RandomForestHyperParameters:
    """Immutable container for Random Forest hyperparameters and performance metrics.

    Attributes:
        max_depth (int): Maximum depth of trees in the forest.
    ...
    def empty(
        ...
        Returns:
            RandomForestHyperParameters: Empty instance with max_depth=0,
            n_estimators=0, f1_score=None.
        """
        return cls(
            max_depth=0,
            n_estimators=0,
    ...
    def from_search_cv(
        ...
        Args:
            search_cv (GridSearchCV or RandomizedSearchCV): Fitted GridSearchCV or 
            RandomizedSearchCV object.

        Returns:
            RandomForestHyperParameters: Hyperparameters from best estimator.
        """
...
class RandomForest(ModelSpecification):
    ...
    importance analysis, and test set evaluation for random forest classifiers.
    ...
    Attributes:
        ...
        hyperparameters (RandomForestHyperParameters): Best params from CV.
        validation_results (ValidationResults): Validation predictions (initialized as empty).
        validation_model_feature_importances (ModelFeatureImportance): Feature importance.
        test_model (RandomForestClassifier): Model for test set evaluation (initialized as sentinel).
    ...
    Example:
        ...
        ... )
        >>> rf.tune_hyperparameters()  # Workflow methods update attributes
        >>> rf.validate_model(ModelBalancing.Auto_Balanced)
    ...
    def __init__(
        ...
        self.hyperparameters = RandomForestHyperParameters.empty()
    ...
    def model_from_hyperparameters(
        ...
        """Create a RandomForestClassifier from hyperparameters dictionary.

        Factory method that constructs a configured RandomForestClassifier instance
        using hyperparameters from a dictionary. The returned classifier is configured
        but not yet trained.

        Args:
            params (dict): Hyperparameters with keys 'max_depth' and 'n_estimators'.
        ...
        Returns:
            RandomForestClassifier: Configured but untrained classifier.

        Example:
            >>> params = {'max_depth': 10, 'n_estimators': 100}
            >>> model = RandomForest.model_from_hyperparameters(params, random_state=42, class_weight='balanced')
    ...
    def optimal_threshold(
        ...
        Args:
            ...
            model (RandomForestClassifier): Untrained random forest classifier to optimize.
    ...
    def tune_hyperparameters(
        ...
        Trains multiple random forest models with different hyperparameter combinations
        ...
        """
        search_cv_model = RandomForestClassifier(
            random_state=self.random_state,
            class_weight=self.class_weight  # type: ignore[arg-type]
        )
        ...
        self.hyperparameters = RandomForestHyperParameters.from_search_cv(
            search_cv)
    ...
    def validate_model(
        ...
        self.hyperparameters = RandomForestHyperParameters(
            max_depth=self.hyperparameters.max_depth,
            n_estimators=self.hyperparameters.n_estimators,
        ...
        if include_in_report:
            train_header = f'{header} (Training Set)'.center(REPORT_WIDTH, '-')
            valid_header = f'{header} (Validation Set)'.center(
                REPORT_WIDTH, '-')
            report_text += f'''
{train_header}
...
'''
            ...
            report_text += self.validation_model_feature_importances.info()
    ...
    def calc_best_top_n(
        ...
        for n in range(feature_importance.threshold_80_idx, feature_importance.threshold_95_idx + 1):
            ...
            top_n_rf.tune_hyperparameters()

            mc, t = top_n_rf.validate_model(
                model_balancing=model_balancing,
                include_in_report=False,
                header=f'Random Forest - Top {n} Features'
            )
            ...
            if isinstance(top_n_rf.hyperparameters.f1_score_train, float):
                f1_score_train = top_n_rf.hyperparameters.f1_score_train

            if isinstance(top_n_rf.hyperparameters.f1_score_valid, float):
                f1_score_valid = top_n_rf.hyperparameters.f1_score_valid
            ...
            top_n_result = [
                f'Top {n}',
                top_n_rf.hyperparameters.f1_score_cv,
                f1_score_train,
                f1_score_valid,
                top_n_rf.hyperparameters.model_generalization.name,
                top_n_rf.hyperparameters.max_depth,
                top_n_rf.hyperparameters.n_estimators,
            ]
...
'''
    ...
    def evaluate_model(
        ...
        Executes the full workflow: GridSearchCV or RandomizedSearchCV for hyperparameters, 
        validation with best params, and feature selection to find optimal feature subset.
        ...
        mc, t = self.validate_model(
            ...
            header=f'Random Forest ({model_balancing.name} - All Features)'
        )
    ...
    def prepare_test_model(
            ...
            include_in_report: bool = True,
            header: str = 'Random Forest (Validation Set)'
    ) -> str:
...
'''
    ...
    def evaluate_test_set(
        ...
        text_result += rf.prepare_test_model(
            model_configuration=model_configuration,
            include_in_report=True,
            header='Best Model (Validation Set)'
        )

        predicted_test = rf.test_model.predict(
